	...
	for(k = 0;k<nbSommets;k++){
		...
		if(k%2 == 0){
			pStar->m_pVertices[k] =(SDL_Point){
				.x = ptCenter.x + iRadius*cos(k*2*M_PI/(nbSommets)),
				.y = ptCenter.y + iRadius*sin(k*2*M_PI/(nbSommets)),
			};
		}
		else{
			pStar->m_pVertices[k] =(SDL_Point){
				.x = ptCenter.x + iRadius*STAR_INTERNAL_RADIUS_RATIO*cos(k*2*M_PI/(nbSommets)), // Les sommets impaire sont dans le rayon grand
				.y = ptCenter.y + iRadius*STAR_INTERNAL_RADIUS_RATIO*sin(k*2*M_PI/(nbSommets)),
			};
		}
	}
...
t_star*StarDraw(const t_star*pStar, SDL_Renderer*pRenderer){
	...
	SDL_RenderDrawPoints(pRenderer, pStar->m_pVertices, pStar->m_szBranches*2+1);
...
t_star*StarMove(t_star*pStar, SDL_Rect*pSpace){
	...
	if(pStar->m_ptCenter.x  <= pSpace->x){
		return pStar;
	}
	if(pStar->m_ptCenter.y < pSpace->y){
		return pStar;
	}

	if(pStar->m_ptCenter.x >=pSpace->w + pSpace->x ){
		return pStar;
	}
	if(pStar->m_ptCenter.y>=pSpace->h + pSpace->y ){
		return pStar;
	}
